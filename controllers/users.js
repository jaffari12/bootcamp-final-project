...
const getUserPosts = async (req, res, next) => {
  try {
    const { id } = req.params;
    const posts = await Post.find({ _userId: id, isDeleted: false }).populate(
      "_userId"
    );
    res.json({
      success: true,
      msg: `posts of user with user id ${id} retrieved`,
      data: posts,
    });
  } catch (err) {
    next(err);
...
const getUserComments = async (req, res, next) => {
  try {
    ...
    res.json({
      success: true,
      msg: `Comments of user with user id ${id} retrieved`,
      data: comments,
    });
  } catch (err) {
    next(err);
